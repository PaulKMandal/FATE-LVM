...
class FedAVGTrainer(TrainerBase):
    ...
    def train_an_epoch(self, epoch_idx, model, train_set, optimizer, loss):
        ...
        for _batch_iter in to_iterate:
            ...
            batch_data = {"pixel_values":batch_data["pixel_values"],"pixel_mask":batch_data["pixel_mask"],"labels":[batch_label]}
            ...
            if not loss and hasattr(pred, "loss"):
                ...
                batch_loss = pred.loss
                batch_loss_dict = pred.loss_dict
    ...
    def _predict(self, dataset: Dataset):
        ...
        with torch.no_grad():
            for _batch_iter in DataLoader(
                ...
                batch_label={}
                batch_label['boxes'] = torch.tensor(batch_data["boxes"])

                #batch_label['boxes'] = batch_label['boxes'].squeeze().unsqueeze(0)
                batch_label['boxes'] = batch_label['boxes'].squeeze(0)#.unsqueeze(0)
                batch_label['class_labels'] = torch.tensor(batch_data["labels"]).squeeze(0)#.unsqueeze(0)
                batch_label['image_id'] = torch.tensor(batch_data["image_id"])
                batch_label['area'] = torch.tensor(batch_data["area"])
                batch_label['iscrowd'] = torch.tensor(batch_data["iscrowd"])

                pixel_values = torch.tensor(batch_data["image"])
                ...
                batch_data = {"pixel_values":batch["pixel_values"],"pixel_mask":batch["pixel_mask"],"labels":[batch_label]}
            ...
            ret_label={"boxes":[],"class_labels":[],"image_ids":[],"area":[],"iscrowd":[]}

            
            
            for item in labels:
                LOGGER.info("inside ret_label")
                LOGGER.info(item)
                ret_label["boxes"] = ret_label["boxes"].append(item["boxes"])
                ret_label["class_labels"] = ret_label["class_labels"].append(["class_labels"])
                ret_label["image_ids"] = ret_label["image_ids"].append(["image_ids"])
                ret_label["area"] = ret_label["area"].append(["area"])
                ret_label["iscrowd"] = ret_label["iscrowd"].append(["iscrowd"])
        ...
        return dataset.get_sample_ids(), pred_result, labels
    ...
    def format_predict_result(self, sample_ids: List, predict_result,
                              true_result, task_type: str = 'objdet'):

        true_result = true_result
        predict_result = predict_result
        
        id_table = [(id_, Instance(label=l))
                    for id_, l in zip(sample_ids, true_result)]
        score_table = [(id_, pred)
                       for id_, pred in zip(sample_ids, predict_result)]
        return StdReturnFormat(id_table, score_table)
...
class StdReturnFormat(object):
    ...
    def __init__(self, id_table_list, pred_table):
        self.id = id_table_list
        self.pred_table = pred_table

    def __call__(self,):
        return self.id, self.pred_table